...
namespace slog {
    ...
    public:
        SyslogClient(Facility facility, std::string hostname, std::string app_name) noexcept
            : process_id_(getpid())
            , facility_(facility)
    ...
    private:
        void createDatagram(std::ostream &ss, Severity severity, clock_type::time_point timestamp,
                            const std::string &message) {
            ...
            // PRI
            {
                auto pri_value = static_cast<int>(facility_) * 8 + static_cast<int>(severity);
                ss << "<" << pri_value << ">";
            }
            ...
            // TIMESTAMP
            {
                ...
                ss << std::setw(4) << std::setfill('0') << year << '-' << std::setw(2)
                   << std::setfill('0') << month << '-' << std::setw(2) << std::setfill('0') << date
                   << 'T' << std::setw(2) << std::setfill('0') << hour << ':' << std::setw(2)
                   << std::setfill('0') << minute << ':' << std::setw(2) << std::setfill('0')
                   << second << "+00:00";
            }
